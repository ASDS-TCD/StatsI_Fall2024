...
b2 <- .75 # True value for the slopes
n <- 1000 # Sample size
...
rand_vcv <- matrix(0, nrow = 20, ncol = 20) 
diag(rand_vcv) <- 1 

# Create 20 uncorrelated variables 
rand_data <- as.data.frame(rmvnorm(1000, mean = rep(0, times = 20),
                           sigma = rand_vcv))

# Rename columns
colnames(rand_data) <- c("y", "x1", "x2", "x3", "x4", "x5", "x6", "x7", 
 "x8", "x9", "x10", "x11", "x12", "x13", "x14", "x15", "x16", "x17",
 "x18", "x19")

# Estimate model
rand_model <- lm(y ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 +
 x11 + x12 + x13 + x14 + x15 + x16 + x17 + x18 + x19, data = rand_data)
...
train <- sample(1:n, train_size, replace = TRUE)
train_data <- rand_data[train_size, ]
test_data <- rand_data[-train_size, ]

rand_model_train <- lm(formula(rand_model), data = train_data)

summary(rand_model_train)

cv_lp <- numeric(test_size)

for (i in 1:(test_size)){
  cv_lp[i] <- as.numeric(c(1, test_data[i, -1])) %*% coef(rand_model_train)
}

mean(cv_lp)


mean(predict(rand_model, rand_data[, -1]))

mean(predict(rand_model, train_data[, -1]))

mean(predict(rand_model, test_data[, -1]))
